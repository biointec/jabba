...
TString::TString(std::string str) : length(0), buf(NULL)
{
        setSequence(str);
}

TString::TString(std::ifstream& ifs)
{
...
void TString::read(std::ifstream& ifs)
{
...
std::string TString::getSequence() const
{
...
std::string TString::substr(size_t offset, size_t len) const
{
        if (offset >= length)
                return std::string();
