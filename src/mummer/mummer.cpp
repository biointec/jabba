...
void *query_thread(void *arg_) {
  ...
  vector<match_t> matches;
	...
	//needs to be 0 to print to file
  bool print = 0;//arg->skip == 1;
...
void usage(string prog) {
  ...
  cerr << "-b             compute forward_ and reverse complement matches" << endl;
  ...
  cerr << "-c             Report the query position of a reverse complement match relative to the forward_ strand of the query sequence" << endl;
  cerr << endl;
  ...
  cerr << "Find all maximal matches on forward_ and reverse strands" << endl;
